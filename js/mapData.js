 ...
 * Distances chosen so Dorm C → Gym via Cafeteria = 610 m (demo route).
 * @returns {Graph}
 */
function buildCampusGraph() {
  const g = new Graph();

  // Named locations (green circles in mockup)
  ...
  // Intersection waypoints (grey dots)
  ...
  g.addEdge("intersection_lower", "gym", 500, "intersection_lower-gym");
...
/**
 * Resolve closed edges from preset id or custom list.
...
/**
 * Enumerate unique undirected edges for rendering.
 * @param {Graph} graph
...
/**
 * Human-readable path description for console-style output.
 * @param {Graph} graph
 * @param {string[]} path
 * @returns {string}
 */
